...
namespace EStore.Storage
{
    public class DefaultChunkStatisticService : IChunkStatisticService
    {
        ...
        public DefaultChunkStatisticService(IScheduleService scheduleService, ILoggerFactory loggerFactory)
        {
        ...
        private void LogChunkStatisticStatus()
        {
            if (_logger.IsDebugEnabled)
            {
                ...
                _logger.DebugFormat("write:{0}, unmanagedCacheRead:{1}, localCacheRead:{2}, fileRead:{3}", bytesWriteStatus, unmanagedReadStatus, cachedReadStatus, fileReadStatus);
            }
        }
